...
public final class ScriptValidator {
    public List<String> validate(NewsScript s,FactPackage facts,int targetSeconds){
        ...
        if(s.segments()!=null&&(s.segments().size()<7||s.segments().size()>10))problems.add("expected 7-10 narration segments");
    ...
    public static int minWords(int targetSeconds){return Math.max(70,(int)Math.ceil(Math.max(1,targetSeconds)*2.25));}
    public static int maxWords(int targetSeconds){return Math.max(minWords(targetSeconds)+20,(int)Math.ceil(Math.max(1,targetSeconds)*2.75));}

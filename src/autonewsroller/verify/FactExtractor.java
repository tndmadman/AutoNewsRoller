    ...
    public List<FactClaim>extract(StoryCluster c){
        ...
        for(Article a:c.articles){
            ...
            for(String sentence:Text.sentences(a.bodyText())){
                ...
                if(clean.length()<28||clean.length()>320)continue;
                candidate.add(clean);
                if(++bodyAdded>=14)break;
            }

            for(String raw:candidate){
                String s=Text.clean(raw);
                if(s.length()<18||s.length()>360)continue;
                String k=Text.normalize(s);if(k.isBlank())continue;
        ...
        out.sort(Comparator
                .comparingInt((FactClaim f)->f.supportingSources().size()).reversed()
